...
import random
import os
from TestCases.aws_config_tests import *
...
def test_aws_check_if_config_rule_created_backup(region):
    config_client = assumeRoleSession.client('config', region_name=region)
    assert aws_check_if_config_rule_created(config_client, "platform_Auto-Backup-Tag") == True

@pytest.mark.parametrize('region', regions)
def test_aws_check_if_config_rule_created_Autotag(region):
    config_client = assumeRoleSession.client('config', region_name=region)
    assert aws_check_if_config_rule_created(config_client, "platform_Auto-Tag") == True