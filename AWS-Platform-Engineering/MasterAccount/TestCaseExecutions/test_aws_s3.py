...
@pytest.mark.parametrize('bucket_name', parameters_data['S3']['ListOfEncryptedBuckets'])
def test_aws_s3_check_if_bucket_is_encrypted(bucket_name):
    assert aws_s3_check_if_bucket_is_encrypted(s3_client, bucket_name) == True
...
@pytest.mark.parametrize('bucket_name', parameters_data['S3']['ListOfBucketsWithVersioning'])
def test_aws_s3_check_if_bucket_is_having_versioning_enabled(bucket_name):
    assert aws_s3_check_if_bucket_is_having_versioning_enabled(s3_client, bucket_name) == True

@pytest.mark.parametrize('bucket_name', parameters_data['S3']['ListOfBucketsWithLifecycleConfiguration'])
def test_aws_s3_check_if_bucket_is_having_lifecycle_configuration(bucket_name):
    assert aws_s3_check_if_bucket_is_having_lifecycle_configuration(s3_client, bucket_name) == True
...
@pytest.mark.parametrize('bucket_name', parameters_data['S3']['ListOfBucketsWithBucketPolicies'])
def test_aws_s3_check_if_bucket_is_having_bucket_policy(bucket_name):
    assert aws_s3_check_if_bucket_is_having_bucket_policy(s3_client, bucket_name) == True