...
################################################################################################
#
# This is AWS provided code taken from:
# https://github.com/awslabs/aws-config-rules/blob/master/python/IAM_POLICY_REQUIRED/IAM_POLICY_REQUIRED.py
#
# Any Shell specific customisations have been clearly commented below.
#
################################################################################################
################################################################################################
#
...
def should_ignore_config_item(config_item, ignored_roles, ignored_users):
    # THE FINAL OR CLAUSE IS AN ADDITIONAL SHELL REQUIREMENT TO EXCLUDE SSO ROLES FROM THE COMPLIANCE MANAGEMENT
    return (config_item['resourceType'] == 'AWS::IAM::Role' and config_item['resourceName'] in ignored_roles) \
            ...
            or (config_item['ARN'].rsplit("/")[1] == 'aws-reserved')
