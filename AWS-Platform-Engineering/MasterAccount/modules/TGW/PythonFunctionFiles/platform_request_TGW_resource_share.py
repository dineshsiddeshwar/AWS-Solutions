...
class RequestTGWResourceShare(object):
    """
    # Class: RequestTGWResourceShare
    # Description: Request the TGW Resource to be shared in the child account
    """
    ...
    def __init__(self, event, context):
        ...
        self.result = {'tgw_resource_share_request_api': {}}
        self.res_dict = {}
        try: 
            ...
            self.result.update({'tgw_resource_share_request' : FAILED})

 
    def request_TGW_resource_share(self):     
        """
        # Takes request body
    ...
    def request_resource_share(self):
        try:
            logger.info("proceeding to request resource share in {}".format(self.region))
            ...
            if len(ram_response['resources']) > 0:
                for item in ram_response['resources']:
                    if item['arn'] == self.resourceArn and item['status'] == 'AVAILABLE':
                        tgw_available = TRUE
                        logger.info("TGW Resource is already shared. Hence skipping...")
                        self.res_dict[self.region].update({'status' : PASSED})
                        self.update_result()
            if tgw_available == FALSE:
                #Requesting tgw resource share
                logger.info("Proceeding to POST API call...")
                self.request_body = {'account': {}}
                self.request_body['account']['account_id'] = self.account_id
                self.request_body['account']['region'] = self.region
                self.request_body['account']['environment'] = self.environment
                logger.info(json.dumps(self.request_body))
                headers = {'x-api-key': self.key}
                logger.info("Entered Post with '{0}".format(self.request_body))
                request_response = requests.post(url=self.url, data=json.dumps(self.request_body), headers=headers)
                logger.info(request_response)
                status_code = request_response.status_code
                status_message = request_response.json()
                if status_code == 200:
                    logger.info("Successfully requested tgw resource share.")
    ...
    def update_result(self):
        try:
            if self.region == 'us-east-1':
                self.result['tgw_resource_share_request_api'].update({'us-east-1': self.res_dict[self.region]})
            elif self.region == 'eu-west-1':
                self.result['tgw_resource_share_request_api'].update({'eu-west-1':self.res_dict[self.region]})
            elif self.region == 'ap-southeast-1':
                self.result['tgw_resource_share_request_api'].update({'ap-southeast-1':self.res_dict[self.region]})
            return self.result
        except Exception as exception:
            logger.error(str(exception))
            self.result['tgw'].update({'tgw_resource_share_request_update_result' : FAILED})
            return self.result 
    ...
    def get_ssm_param(self,parameter_name):
        ...
        try:
            ...
            self.result.update({'tgw_resource_share_request_get_parameter' : FAILED})
            return self.result
    ...
    def alert_operations(self,account_id, region_name,request_id, message):
        try:
            ...
            body_html = body_html = """<html>
                    ...
                    <p></p>
                    <p style="font-family:'Arial Nova'">The following error occurred during Request TGW resource share operation..</p>
                    <ul>
            ...
            send_mail_response = self.ses_client.send_email(
                ...
                Message={
                    'Subject': {
                        'Data':' ALERT: Request TGW Resource Share Failed'
                    },
                    'Body': {
                        'Text': {
                            'Data': body_text
...
def lambda_handler(event, context):
    ...
    result.update(event)
    try:
        tgw_integration = RequestTGWResourceShare(event, context)
        output = tgw_integration.request_TGW_resource_share()
        result.update(output)
        return result
    except Exception as exception:
        logger.error(str(exception))
        result.update({'tgw_resource_share_request' : FAILED})
        return result
