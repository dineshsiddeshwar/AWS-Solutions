...
class TGWAttachment(object):
    ...
    def __init__(self, event, context):
        ...
        try: 
            ...
            self.account_type = "Private" if "Private" in event['ProvisionedProduct']['OU'] else ("Hybrid"  if "Hybrid" in event['ProvisionedProduct']['OU'] else "Public")
        ...
        try:
            ...
            if tgw_attachment == FALSE:
                ...
                if len(tgw_attachment_response['TransitGatewayVpcAttachment']) > 0:
                    ...
                    self.alert_operations(self.account_id, self.region,self.event['RequestEventData']['RequestNo'], message)
            return self.result
        except Exception as exception:
            logger.error(str(exception))
            self.res_dict[self.region].update({'create_tgw_attachment' : FAILED})
