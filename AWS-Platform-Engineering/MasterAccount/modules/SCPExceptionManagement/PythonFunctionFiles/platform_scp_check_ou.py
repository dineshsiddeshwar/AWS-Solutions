...
def lambda_handler(event, context):
    ...
    try:
        ...
        if org_dict[ou_id] == "private_exc_OU" or org_dict[ou_id] == "public_exc_OU":
            LOGGER.info("The account {0} has moved to Exception OU :- ".format(event['accountNumber']))
            check_ou_status = 'SUCCESS'
        else:
            LOGGER.info("The account {0} has not moved to Exception OU :- ".format(event['accountNumber']))
            check_ou_status = 'FAILED'
        temp_dict = {"check_ou_status": check_ou_status}
        modified_event.update(temp_dict)
        return modified_event
    except Exception as exception:
        print("Error in Lambda Handler", exception)
        event.update({"check_ou_status": "FAILED","Error_message": str(exception)})
        return event