...
class InstallCloudhealthagent(object):
    ...
    def __init__(self, event, context):
        ...
        try:
            ...
            self.child_assume_role_session = boto3.Session(child_access_keyid, child_secret_access_key,
                                                           child_session_token)
            self.association_id = ""
            self.target_tag = "tag:platform_cloudhealth_windows"
    ...
    def create_ssm_association_public(self):
        '''
        Create the SSM Association in the child account to Run remote script from S3 for cloudhealth installation for windows
        '''
        try:
            path = self.event['SSMParametres']['platform_win_dirpath']
            execution_time = self.event['SSMParametres']['platform_execution_timeout']
            script_path = self.event['SSMParametres']['platform_pub_ch_winpath']
            file_name = self.event['SSMParametres']['platform_ch_win_filename']
            ...
            for region in self.public_region:
                ...
                if (
                not self.verify_association(self.ssm_childaccount_client, "platform_InstallCloudhealthagent-Windows")):
                    response = self.ssm_childaccount_client.create_association(
                        ...
                        AssociationName='platform_InstallCloudhealthagent-Windows'
                    )
                    print("SSM Association for cloudhealth installation for windows has been created successfully!!!")
                    self.res_dict['cloudhealth_association_creation_pub_win'] = "PASSED"
                else:
                    ...
                    print("SSM Association for cloudhealth installation for Windows has been updated successfully!!!")
                    self.res_dict['cloudhealth_association_creation_pub_win'] = "PASSED"
        except Exception as exception:
            print("Exception occurred while creating SSM Association for Falcon windows {}".format(str(exception)))
            self.res_dict['cloudhealth_association_creation_pub_win'] = "FAILED"
            return self.res_dict
    def create_ssm_association_private(self):
        ...
        try:
            path = self.event['SSMParametres']['platform_win_dirpath']
            execution_time = self.event['SSMParametres']['platform_execution_timeout']
            script_path = self.event['SSMParametres']['platform_pvt_ch_winpath']
            file_name = self.event['SSMParametres']['platform_ch_win_filename']
            source_info = ['{"path": ' + script_path + '}']
            source_type = ['S3']
            command_line = [file_name]
            ...
            for region in self.private_region:
                ...
                not self.verify_association(self.ssm_childaccount_client, "platform_InstallCloudhealthagent-Windows")):
                    ...
                    print("SSM Association for cloudhealth installation for windows has been created successfully!!!")
                    self.res_dict['cloudhealth_association_creation_pvt_win'] = "PASSED"
                else:
                    response = self.ssm_childaccount_client.update_association(
                        AssociationId=self.association_id,
                        Name="AWS-RunRemoteScript",
            ...
            print("Exception occurred while creating SSM Association for Falcon windows {}".format(str(exception)))
            self.res_dict['cloudhealth_association_creation_pvt_win'] = "FAILED"
            return self.res_dict
    ...
    def verify_association(self, ssm_childaccount_client, association_name):
        ...
        try:
            ...
            if (response['Associations'].__len__() > 0):
                ...
                self.res_dict['Cloudhealth_association_verification_win'] = "PASSED"
                return True
            else:
                self.res_dict['Cloudhealth_association_verification_win'] = "PASSED"
                return False
            ...
            self.res_dict['Cloudhealth_association_verification_win'] = "FAILED"
            return self.res_dict
