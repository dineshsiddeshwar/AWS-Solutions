...
def read_aacount():
    ...
    try:
        ...
        response = dynamo_table.scan(ProjectionExpression='AccountNumber,#State_ref',
        ...
        if (result):
            for item in result:
                if item['State'] == 'Active':
                    account_list.append(item['AccountNumber'])
...
def get_platform_type(account_list):
    ...
    try:
        ...
        for account in account_list:
            response = child_account_instances_details(account, session_client, sts_client, ec2_client, global_list,
                                                         failed_account)
            if response != str:
                succeeded_accounts.append(account)
        print(global_list)
        make_excel_response = make_excel(global_list)
...
def child_account_instances_details(account_number, session_client, sts_client, ec2_client_session, global_list,
                                      failed_account):
    try:
        ...
        for region in regions:
            ...
            ec2_response = ec2_child_client.describe_instances(MaxResults=999)
            if ec2_response['Reservations']:
                for item in ec2_response['Reservations']:
                    instance = {}
                    for elements in item['Instances']:
                        image_id = elements['ImageId']
                        image_details = ec2_child_client.describe_images(ImageIds=[image_id])
                        for item in image_details:
                            temp = image_details[item]
                            for temp_items in temp:
                                if 'Description' in temp_items:
                                    instance['Description'] = temp_items['Description']
                                    instance['PlatformType'] = temp_items['PlatformDetails']
                        ...
                        if 'PublicIpAddress' in elements:
                            ...
                            instance['PublicIpAddress'] = 'Not Available'
                        temp_az = elements['Placement']
                        instance['AvailabilityZone'] = temp_az['AvailabilityZone']
                        instance['AccountId'] = account_number
                        instance['InstanceId'] = elements['InstanceId']
                        instance['InstanceType'] = elements['InstanceType']
                        instance['PrivateIpAddress'] = elements['PrivateIpAddress']
                        global_list.append(instance)
        return global_list
        ...
        LOGGER.error(ex)
        failed_account.append(account_number)
...
def make_excel(input_data):
    ...
    try:
        ...
        lambda_file_name = "/tmp/" + current_year + current_month + "list_operating_systems" + ".csv"
        column_names = ['InstanceId', 'AccountId','AvailabilityZone', 'PlatformType', 'ImageName','Description', 'InstanceType', 'PublicIpAddress',
                        'PrivateIpAddress']
...
def send_mail(file_name):
    try:
        ...
        body_text = "Hello\r\nPlease find the attached Operating system details " + "Report for AWS@Shell" \
        ...
        <p style="font-family:'Futura Medium'">Please find the attached OS platform Report for AWS@Shell.</p>
        ...
        try:
            ...
            mail_subject = "OS Platform Report for " + "-" + current_month + "-" + current_year
            ...
            LOGGER.info(" Mail has beed successfully sent")
