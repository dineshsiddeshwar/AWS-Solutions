...
class InstallRapid7agent(object):
    ...
    def __init__(self, event, context):
        ...
        try:
            ...
            self.child_assume_role_session = boto3.Session(child_access_keyid, child_secret_access_key,
                                                           child_session_token)
            self.association_id = ""
            self.target_tag = "tag:platform_rapid7_linux"
    ...
    def create_ssm_association_public(self):
        '''
        Create the SSM Association in the child account to Run remote script from S3 for rapid7 installation
        '''
        try:
            path = self.event['SSMParametres']['platform_linux_dirpath']
            execution_time = self.event['SSMParametres']['platform_execution_timeout']
            script_path = self.event['SSMParametres']['platform_pub_rapid7_linuxpath']
            file_name = self.event['SSMParametres']['platform_rapid7_linux_filename']
            ...
            for region in self.public_region:
                ...
                if (not self.verify_association(self.ssm_childaccount_client, "platform_InstallRapid7agent-Linux")):
                    response = self.ssm_childaccount_client.create_association(
                        ...
                        AssociationName='platform_InstallRapid7agent-Linux'
                    )
                    print("SSM Association for rapid7 installation for Linux has been created successfully!!!")
                    self.res_dict['rapid7_association_creation_pub_linux'] = "PASSED"
                else:
                    ...
                    print("SSM Association for rapid7 installation for Linux has been updated successfully!!!")
                    self.res_dict['rapid7_association_creation_pub_linux'] = "PASSED"
        except Exception as exception:
            print("Exception occurred while creating SSM Association for rapid7 linux {}".format(str(exception)))
            self.res_dict['rapid7_association_creation_pub_linux'] = "FAILED"
            return self.res_dict
    ...
    def create_ssm_association_private(self):
        ...
        try:
            path = self.event['SSMParametres']['platform_linux_dirpath']
            execution_time = self.event['SSMParametres']['platform_execution_timeout']
            script_path = self.event['SSMParametres']['platform_pvt_rapid7_linuxpath']
            file_name = self.event['SSMParametres']['platform_rapid7_linux_filename']
            source_info = ['{"path": ' + script_path + '}']
            source_type = ['S3']
            command_line = [file_name]
            ...
            for region in self.private_region:
                ...
                if (not self.verify_association(self.ssm_childaccount_client, "platform_InstallRapid7agent-Linux")):
                    ...
                    print("SSM Association for rapid7 installation for Linux has been created successfully!!!")
                    self.res_dict['rapid7_association_creation_priv_linux']= "PASSED"
                else:
                    response = self.ssm_childaccount_client.update_association(
                        AssociationId=self.association_id,
                        Name="AWS-RunRemoteScript",
            ...
            print("Exception occurred while creating SSM Association for rapid7 linux {}".format(str(exception)))
            self.res_dict['rapid7_association_creation_priv_linux']= "FAILED"
            return self.res_dict
    ...
    def verify_association(self, ssm_childaccount_client, association_name):
        ...
        try:
            ...
            if (response['Associations'].__len__() > 0):
                ...
                self.res_dict['rapid7_association_verification_linux'] = "PASSED"
                return True
            ...
            self.res_dict['rapid7_association_verification_Linux'] = "FAILED"
            return self.res_dict


def lambda_handler(event, context):
    """"
    Lamda handler that calls the function to Create SSM Association to Install rapid7 Agent
    """
    try:
        result_values = {}
