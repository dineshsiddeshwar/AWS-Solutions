'''
Install Flexera Agent on Windows Servers
'''
import logging
import random
import json
...
class InstallFlexeraagent(object):
    '''
    Create SSM Association to install Flexera Agent on windows
    '''
    ...
    def __init__(self, event, context):
        ...
        try:
            ...
            self.child_assume_role_session = boto3.Session(child_access_keyid, child_secret_access_key,
                                                           child_session_token)
            self.association_id = ""
            self.target_tag = "tag:platform_flexera_windows"
    ...
    def update_s3_bucket_policy(self):
        ...
        to the script
        '''
        principal_str = "arn:aws:iam::{}:role/*".format(
            self.child_account_number)
        try:
            bucket_name = self.event['SSMParametres']['platform_agent_bucket']
            bucket_policy = json.loads(
                self.s3_client.get_bucket_policy(Bucket=bucket_name)['Policy'])
            ...
            print("Exception occurred while updating bucket policy {}".format(
                str(exception)))
    ...
    def create_ssm_association_public(self):
        '''
        Create the SSM Association in the child account to Run Remote Powershell script from S3
        '''
        try:
            response = self.ssm_client.get_parameters(
                Names=["platform_pub_flexera_winpath", "platform_execution_timeout",
                       "platform_win_dirpath", "platform_flexera_win_filename"],
                WithDecryption=True)
            for values in response['Parameters']:
                ...
                elif values['Name'] == 'platform_pub_flexera_winpath':
                    script_path = values['Value']
                elif values['Name'] == 'platform_flexera_win_filename':
            ...
            for region in self.public_region:
                ...
                if (not self.verify_association(self.ssm_childaccount_client, "platform_InstallFlexeraAgent-Windows")):
                    response = self.ssm_childaccount_client.create_association(
                        ...
                        AssociationName='platform_InstallFlexeraAgent-Windows'
                    )
                    print(
                        "SSM Association for Flexera installation for windows has been created successfully!!!")
                    self.res_dict['flexera_association_creation_pub_win'] = "PASSED"
                else:
                    ...
                    print(
                        "SSM Association for Flexera installation for Windows has been updated successfully!!!")
                    self.res_dict['flexera_association_creation_pub_win'] = "PASSED"
        except Exception as exception:
            print("Exception occurred while creating SSM Association for Flexera windows {}".format(
                str(exception)))
            self.res_dict['flexera_association_creation_pub_win'] = "FAILED"
            return self.res_dict

    def file_path(self, region):
        if region == "us-east-1":
            flexera_winpath = "platform_pvt_flexera_winpath_us"
            return flexera_winpath
        elif region == "eu-west-1":
            flexera_winpath = "platform_pvt_flexera_winpath_eu"
            return flexera_winpath
    ...
    def create_ssm_association_private(self):
        ...
        try:
            for region in self.private_region:
                flexera_winpath = self.file_path(region)
                print(flexera_winpath)

                
                path = self.event['SSMParametres']['platform_win_dirpath'] 
                execution_time = self.event['SSMParametres']['platform_execution_timeout']
                script_path = self.event['SSMParametres'][flexera_winpath]
                file_name = self.event['SSMParametres']['platform_flexera_win_filename']
                source_info = ['{"path": ' + script_path + '}']
                source_type = ['S3']
                command_line = [file_name]
                working_directory = [path]
                ...
                if (not self.verify_association(self.ssm_childaccount_client, "platform_InstallFlexeraAgent-Windows")):
                    ...
                    print(
                        "SSM Association for Flexera installation for windows has been created successfully!!!")
                    self.res_dict['flexera_association_creation_pvt_win'] = "PASSED"
                else:
                    response = self.ssm_childaccount_client.update_association(
                        AssociationId=self.association_id,
                        Name="AWS-RunRemoteScript",
            ...
            print("Exception occurred while creating SSM Association for Flexera windows {}".format(
                str(exception)))
            self.res_dict['flexera_association_creation_pvt_win'] = "FAILED"
            return self.res_dict
    ...
    def verify_association(self, ssm_childaccount_client, association_name):
        ...
        try:
            ...
            if (response['Associations'].__len__() > 0):
                ...
                self.res_dict['flexera_association_verification_win'] = "PASSED"
                return True
            else:
                self.res_dict['flexera_association_verification_win'] = "PASSED"
                return False
        except Exception as exception:
            print("Exception occurred while verifying SSM Association {}".format(
                str(exception)))
            self.res_dict['flexera_association_verification_win'] = "FAILED"
            return self.res_dict
...
def lambda_handler(event, context):
    ...
    try:
        ...
        create_ssm_obj = InstallFlexeraagent(event, context)
