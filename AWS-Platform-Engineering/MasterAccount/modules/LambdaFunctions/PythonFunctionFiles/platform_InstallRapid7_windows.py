'''
Create SSM Association for InstallRapid7agent process
'''
import logging
import random
import json
...
class InstallRapid7agent(object):
    """
    Class: InstallRapid7agent
    Description: Prerequisite for InstallRapid7agent proccess
    """
    ...
    def __init__(self, event, context):
        ...
        try:
            ...
            self.child_assume_role_session = boto3.Session(child_access_keyid, child_secret_access_key,
                                                           child_session_token)
            self.association_id = ""
            self.target_tag = "tag:platform_rapid7_windows"
    ...
    def create_ssm_association_public(self):
        '''
        Create the SSM Association in the child account to Run remote script from S3 for Rapid installation for windows
        '''
        try:
            path = self.event['SSMParametres']['platform_win_dirpath']
            execution_time = self.event['SSMParametres']['platform_execution_timeout']
            script_path = self.event['SSMParametres']['platform_pub_rapid7_winpath']
            file_name = self.event['SSMParametres']['platform_rapid7_win_filename']
            ...
            for region in self.public_region:
                ...
                if (not self.verify_association(self.ssm_childaccount_client, "platform_InstallRapid7agent-Windows")):
                    response = self.ssm_childaccount_client.create_association(
                        ...
                        AssociationName='platform_InstallRapid7agent-Windows'
                    )
                    print("SSM Association for Rapid7 installation for windows has been created successfully!!!")
                    self.res_dict['rapid7_association_creation_pub_win'] = "PASSED"
                else:
                    ...
                    print("SSM Association for rapid7 installation for Windows has been updated successfully!!!")
                    self.res_dict['rapid7_association_creation_pub_win'] = "PASSED"
        except Exception as exception:
            print("Exception occurred while creating SSM Association for rapid7 windows {}".format(str(exception)))
            self.res_dict['rapid7_association_creation_pub_win'] = "FAILED"
            return self.res_dict
    ...
    def create_ssm_association_private(self):
        ...
        try:
            path = self.event['SSMParametres']['platform_win_dirpath']
            execution_time = self.event['SSMParametres']['platform_execution_timeout']
            script_path = self.event['SSMParametres']['platform_pvt_rapid7_winpath']
            file_name = self.event['SSMParametres']['platform_rapid7_win_filename']
            source_info = ['{"path": ' + script_path + '}']
            source_type = ['S3']
            command_line = [file_name]
            ...
            for region in self.private_region:
                ...
                if (not self.verify_association(self.ssm_childaccount_client, "platform_InstallRapid7agent-Windows")):
                    ...
                    print(
                        "SSM Association for Rapid7 installation for windows has been created successfully!!!")
                    self.res_dict['rapid7_association_creation_pvt_win'] = "PASSED"
                else:
                    response = self.ssm_childaccount_client.update_association(
                        AssociationId=self.association_id,
                        Name="AWS-RunRemoteScript",
                    ...
                    print(
                        "SSM Association for rapid7 installation for Windows has been updated successfully!!!")
                    self.res_dict['rapid7_association_creation_pvt_win'] = "PASSED"
        except Exception as exception:
            print("Exception occurred while creating SSM Association for rapid7 windows {}".format(
                str(exception)))
            self.res_dict['rapid7_association_creation_pvt_win'] = "FAILED"
            return self.res_dict
    ...
    def verify_association(self, ssm_childaccount_client, association_name):
        ...
        try:
            ...
            self.res_dict['rapid7_association_verification_win'] = "FAILED"
            return self.res_dict


def lambda_handler(event, context):
    """"
    Lamda handler that calls the function to Creat SSM Association to Install rapid7 Agent
    """
    try:
        result_values = {}
        result_values.update(event)
        print("Event {}".format(event))
        create_ssm_obj = InstallRapid7agent(event, context)
        ...
        try:
            if 'public' in account_type:
                ...
                print("SSM association creation failed for rapid7 linux or account Might be a platform account")
                return result_values
        except Exception as exception:
