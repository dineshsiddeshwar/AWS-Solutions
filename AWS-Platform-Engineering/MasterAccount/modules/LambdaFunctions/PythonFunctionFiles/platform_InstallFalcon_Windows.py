'''
Create SSM Association for InstallFalconagent process
'''
import logging
import random
import json
...
class InstallFalconagent(object):
    """
    ...
    def __init__(self, event, context):
        ...
        try:
            ...
            self.child_assume_role_session = boto3.Session(child_access_keyid, child_secret_access_key,
                                                           child_session_token)
            self.association_id = ""
            self.target_tag = "tag:platform_falcon_windows"
    ...
    def update_s3_bucket_policy(self):
        ...
        try:
            ...
            if principal_str in principal_list:
                print('Child account roles already updated in s3 bucket policy')
    ...
    def create_ssm_association_public(self):
        '''
        Create the SSM Association in the public child account to Run remote script from S3 for Falcon installation for windows
        '''
        try:
            path = self.event['SSMParametres']['platform_win_dirpath']
            execution_time = self.event['SSMParametres']['platform_execution_timeout']
            script_path = self.event['SSMParametres']['platform_pub_falcon_winpath']
            file_name =self.event['SSMParametres']['platform_falcon_win_filename']
            ...
            for region in self.public_region:
                ...
                if (not self.verify_association(self.ssm_childaccount_client, "platform_InstallFalconAgent-Windows")):
                    response = self.ssm_childaccount_client.create_association(
                        ...
                        AssociationName='platform_InstallFalconAgent-Windows'
                    )
                    print("SSM Association for Falcon installation for windows has been created successfully!!!")
                    self.res_dict['falcon_association_creation_pub_win'] = "PASSED"
                else:
                    ...
                    print("SSM Association for Falcon installation for Windows has been updated successfully!!!")
                    self.res_dict['falcon_association_creation_pub_win'] = "PASSED"
        except Exception as exception:
            print("Exception occurred while creating SSM Association for Falcon windows {}".format(str(exception)))
            self.res_dict['falcon_association_creation_pub_win'] = "FAILED"
            return self.res_dict
    
    def create_ssm_association_private(self):
        '''
        Create the SSM Association in the private child account to Run remote script from S3 for Falcon installation for windows
        '''
        try:
            """response = self.ssm_client.get_parameters(
                Names=["platform_pvt_falcon_winpath", "platform_execution_timeout",
                       "platform_win_dirpath", "platform_falcon_win_filename"],
                WithDecryption=True)
            for values in response['Parameters']:
                if values['Name'] == 'platform_win_dirpath':
                    path = values['Value']
                elif values['Name'] == 'platform_execution_timeout':
                    execution_time = values['Value']
                elif values['Name'] == 'platform_pvt_falcon_winpath':
                    script_path = values['Value']
                elif values['Name'] == 'platform_falcon_win_filename':
                    file_name = values['Value']"""
            path = self.event['SSMParametres']['platform_win_dirpath']
            execution_time = self.event['SSMParametres']['platform_execution_timeout']
            script_path = self.event['SSMParametres']['platform_pvt_falcon_winpath']
            file_name = self.event['SSMParametres']['platform_falcon_win_filename']
            source_info = ['{"path": ' + script_path + '}']
            source_type = ['S3']
            command_line = [file_name]
            ...
            for region in self.private_region:
                self.ssm_childaccount_client = self.child_assume_role_session.client(
                ...
                if (not self.verify_association(self.ssm_childaccount_client, "platform_InstallFalconAgent-Windows")):
                    ...
                    print(
                        "SSM Association for Falcon installation for windows has been created successfully!!!")
                    self.res_dict['falcon_association_creation_pvt_win'] = "PASSED"
                else:
                    response = self.ssm_childaccount_client.update_association(
                        AssociationId=self.association_id,
                        Name="AWS-RunRemoteScript",
                    ...
                    print(
                        "SSM Association for Falcon installation for Windows has been updated successfully!!!")
                    self.res_dict['falcon_association_creation_pvt_win'] = "PASSED"
        except Exception as exception:
            print("Exception occurred while creating SSM Association for Falcon windows {}".format(
                str(exception)))
            self.res_dict['falcon_association_creation_pvt_win'] = "FAILED"
            return self.res_dict
    ...
    try:
        ...
        try:
            if 'public' in account_type:
                ...
                print("Account might be a platform account")
