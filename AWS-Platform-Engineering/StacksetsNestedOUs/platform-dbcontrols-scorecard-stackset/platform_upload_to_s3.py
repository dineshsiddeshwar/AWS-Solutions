...
def create_zip_for_each_py_file(folder_path):
      try:
            ...
            layer_zip_files = [f for f in files if f.endswith('.zip')]

            # upload zip file for layers
            for layer_zip_file in layer_zip_files:
                  layer_zip_path = os.path.join(folder_path, layer_zip_file)
                  upload_stackset_resources_to_s3(layer_zip_path, layer_zip_file) 
