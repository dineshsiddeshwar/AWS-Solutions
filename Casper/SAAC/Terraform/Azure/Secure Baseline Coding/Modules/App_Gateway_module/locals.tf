locals {
  ...
  default_name = lower("${local.name_prefix}${var.stack}-${var.location_short}-${var.environment}")
  ...
  frontend_priv_ip_configuration_name = var.frontend_priv_ip_configuration_name != "" ? var.frontend_priv_ip_configuration_name : join("-", [local.default_name, "private-frontipconfig"])
