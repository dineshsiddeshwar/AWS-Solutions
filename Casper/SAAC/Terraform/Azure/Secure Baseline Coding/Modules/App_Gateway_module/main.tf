...
resource "azurerm_application_gateway" "app_gateway" {
  ...
  dynamic "frontend_ip_configuration" {
    for_each = var.appgw_private ? ["true"] : []
    content {
  ...
  dynamic "waf_configuration" {
    for_each = local.enable_waf ? ["true"] : []
    content {
  ...
  dynamic "autoscale_configuration" {
    for_each = var.autoscaling_parameters != null ? [var.autoscaling_parameters] : []
    content {
  ...
  # Backend HTTP settings for Https protocol must specify a trusted root certificate if you are using a self-signed cert for the backend.
  # Backend HTTP settings with trusted root certificate must either specify Hostname field or set PickHostNameFromBackendAddress to true.
  #
  ...
  dynamic "backend_http_settings" {
    ...
    content {
      ...
      dynamic "authentication_certificate" {
        for_each = lookup(backend_http_settings.value, "authentication_certificate_name", null) != null ? [{}] : []
        content {
      ...
      connection_draining {
        enabled           = lookup(backend_http_settings.value, "connection_draining", false)
        drain_timeout_sec = lookup(backend_http_settings.value, "drain_timeout_sec", 300)
      }
    }
  }
  ...
  dynamic "http_listener" {
    ...
    content {
      ...
      frontend_ip_configuration_name = lookup(http_listener.value, "frontend_ip_configuration_name", var.appgw_private ? local.frontend_priv_ip_configuration_name : local.frontend_ip_configuration_name)
      ...
      dynamic "rewrite_rule" {
        ...
        content {
          ...
          dynamic "request_header_configuration" {
            for_each = lookup(rewrite_rule.value, "request_header_configuration", null)
            iterator = request
            content {
              header_name  = request.value.name
              header_value = request.value.value
            }
          }

          dynamic "response_header_configuration" {
            for_each = lookup(rewrite_rule.value, "response_header_configuration", null)
            iterator = request
            content {
              header_name  = request.value.name
              header_value = request.value.value
